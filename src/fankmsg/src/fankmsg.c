...
int main(int argc, char *argv[])
{
  ...
  if (fd < 0) err(EXIT_FAILURE, "open %s", KMSG_PATH);
  ...
  for (;;)
  {
    ...
    if (fdset[0].revents & (POLLIN | POLLHUP)) handle_kmsg(fd);

    // Any notification from Fantom is to exit
    if (fdset[1].revents & (POLLIN | POLLHUP)) break;
  }
