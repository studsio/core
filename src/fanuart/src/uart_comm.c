 ...
 *
 * Modified for Studs by Andy Frank
 */
...
static void record_last_error(int err)
{
    ...
    case EINVAL:
    default:
        log_debug("Got unexpected error: %d", err);
        last_error = "einval";
        break;
    }
}
...
static int uart_config_flowcontrol(int fd, const struct uart_config *config)
{
    ...
    case UART_FLOWCONTROL_SOFTWARE:
        log_debug("software flow control");
        options.c_cflag &= ~CRTSCTS;
        options.c_iflag |= IXON | IXOFF | IXANY;
        break;
    }
              ...
              uart_notify_read notify_read)
{
    ...
    *pport = port;
    ...
    port->fd = -1;
    port->active_mode_enabled = false; //true;
...
int uart_open(struct uart *port, const char *name, const struct uart_config *config)
{
    ...
    if (!uart_path) {
        log_debug("Can't convert '%s' to uart path", name);
        return -1;
    }
    ...
    if (port->fd < 0) {
        log_debug("open failed on '%s'", name);
        record_errno();
        return -1;
    }
    ...
    if (flock(port->fd, LOCK_EX | LOCK_NB) < 0) {
        log_debug("flock failed on '%s'", name);
        record_errno();
        return -1;
    }

    if (uart_config_line(port->fd, config) < 0) {
        log_debug("uart_config_line failed");
        record_errno();
        return -1;
    }

    if (uart_config_flowcontrol(port->fd, config) < 0) {
        log_debug("uart_config_flowcontrol failed");
        record_errno();
        return -1;
    }
...
int uart_configure(struct uart *port, const struct uart_config *config)
{
    ...
    if (config->active != port->active_mode_enabled) {
        if (port->read_pending)
            log_fatal("Elixir is supposed to queue read ops");
...
{
    if (port->write_data)
        log_fatal("Implement write queuing in Elixir!");

    ssize_t written;
    do {
        written = write(port->fd, data, len);
        log_debug("uart_write: wrote %d/%d, errno=%d (%s) data=%p, fd=%d", (int) written, (int) len, errno, strerror(errno), data, port->fd);
...
static void continue_in_progress_write(struct uart *port)
{
    ...
    do {
        ...
        log_debug("continue_in_progress_write: wrote %d/%d, errno=%d (%s)", (int) written, (int) to_write, errno, strerror(errno));
    } while (written < 0 && errno == EINTR);
...
{
    if (port->active_mode_enabled) {
        log_debug("don't call read when in active mode");
    ...
    if (port->read_pending)
        log_fatal("Implement read queuing in Elixir");
...
int uart_drain(struct uart *port)
{
    ...
    if (port->write_data)
        log_fatal("Elixir is supposed to queue write operations");
...
int uart_flush(struct uart *port, enum uart_direction direction)
{
    ...
    if (port->read_pending)
        log_fatal("Elixir is supposed to queue read operations");
